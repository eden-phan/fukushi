...
class DocumentFinancialContractRepository extends BaseRepository
{
    ...
    public function createDocumentFinancialPolicy(array $data)
    {
        return DB::transaction(function () use ($data) {
            $documentFinancialPolicy = $this->_model->create([
                'document_type' => 'financial_management_contract'
            ]);
    ...
    public function getDocumentFinancialPolicy(
        ...
        $document = $this->_model
            ->with($with)
            ->where('document_type', 'financial_management_contract')
            ->first();
