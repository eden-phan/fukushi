...
import { DeleteTrainingDialog } from "@/app/group-home/training/components/dialogs/delete-training-dialog"
...
type ListTrainingProps = {
    data: PaginatedResponse<TrainingProps> | undefined
    loading: boolean
...
export default function ListTraining({ data, loading, onPageChange, onRefresh, currentPage }: ListTrainingProps) {
    const [isOpenDelDialog, setIsOpenDelDialog] = useState<boolean>(false)
    const [selectedItem, setSelectedItem] = useState<TrainingProps | null>(null)
    const router = useRouter()

    const handleView = (item: TrainingProps) => {
        router.push(`/group-home/training/${item.id}`)
    }

    const handleEdit = (item: TrainingProps) => {
        router.push(`/group-home/training/edit/${item.id}`)
    }

    const handleDelete = (item: TrainingProps) => {
                ...
                {loading ? (
                    ...
                    <UITableBody>
                        {data?.data && Array.isArray(data.data) && data.data.length > 0 ? (
                            data.data.map((item: TrainingProps) => (
