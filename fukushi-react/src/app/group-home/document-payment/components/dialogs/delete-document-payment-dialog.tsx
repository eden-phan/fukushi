...
interface DeleteDocumentPaymentDialogProps {
  open: boolean;
  onOpenChange: (isOpen: boolean) => void;
  documentPayment: DocumentPaymentProps | undefined;
  onRefresh: () => void;
}

export function DeleteDocumentPaymentDialog({
  open,
  onOpenChange,
  documentPayment,
  onRefresh,
}: DeleteDocumentPaymentDialogProps) {
  const handleDelete = async () => {
    if (!documentPayment) return;
    try {
      await http.delete(`/document-payment/${documentPayment.id}`);
